...
void resolvePort(int option, char *device) {
    switch (option) {
        ...
        case 4:
        case 5:
        case 6: sprintf(device, "COM%d", option); break;
        case 10:
        case 11:
        case 12:
        case 13: sprintf(device, "USB%d", option - 9); break;
        default: sprintf(device, "Unknown"); break;
    }
}
...
void setDefaultScaleConfig(char *serialPort) {
  if (!scaleConfig.model) {
    ...
    scaleConfig.baudrate = strncmp(serialPort, "COM", 3) == 0 ?
      BAUDRATE_COM : BAUDRATE_USB;
